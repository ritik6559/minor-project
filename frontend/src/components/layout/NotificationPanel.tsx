...
import { Bell, Check, CheckCheck, AlertTriangle, Info, CircleAlert, Inbox } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { useAuth } from "@/context/AuthContext";
import { dashboardPathForRole } from "@/lib/jwt";
import { useNotifications, useMarkAllRead, useMarkRead } from "@/hooks/useNotification";
...
export function NotificationsPanel({ open, onOpenChange }: Props) {
  ...
  useEffect(() => { /* keep lint quiet */ }, [data]);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="flex w-full flex-col p-0 sm:max-w-md">
        <SheetHeader className="flex-row items-center justify-between border-b px-5 py-4 space-y-0">
          ...
          <Button variant="ghost" size="sm" onClick={() => markAll.mutate()} disabled={markAll.isPending || items.every(i => i.is_read)}>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    !n.is_read && "border-l-2 border-accent bg-accent-soft/40"
                  )}
                >
                  <div className={cn(
                    ...
                    (!n.type || n.type === "info") && "bg-info/15 text-info",
                  )}>
